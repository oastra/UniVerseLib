...
// Fetch user, book status, and reservation data
$sql_user_book = "
    SELECT 
        u.id AS user_id, u.username, u.first_name, u.last_name, u.email, u.role, 
        b.id AS book_id, b.title, 
        bs.status, bs.borrowed_date, bs.return_due_date,
        GROUP_CONCAT(r.status) AS reservation_status
    FROM users u
    LEFT JOIN bookstatus bs ON u.id = bs.member_id
    LEFT JOIN books b ON bs.book_id = b.id
    LEFT JOIN reservations r ON u.id = r.user_id AND b.id = r.book_id
    GROUP BY u.id, u.username, u.first_name, u.last_name, u.email, u.role, b.id, b.title, bs.status, bs.borrowed_date, bs.return_due_date
    ORDER BY u.id, b.id";
$result_user_book = $conn->query($sql_user_book);
...
<body>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr data-id="<?php echo htmlspecialchars($row['id']); ?>">
                            ...
                            <td>
                                <img src="<?php echo htmlspecialchars($row['image']); ?>" alt="<?php echo htmlspecialchars($row['title']); ?>" width="50">
                                <input type="file" class="form-control form-control-sm d-none" accept="image/*">
                            </td>
                            <td>
  ...
  <section class="userTable my-5 mx-5">
    <div class="container my-5">
        <div class="row mt-5">
            <div class="col-md-12">
                <h3>Manage Users and Book Status</h3>
                <table class="table table-striped">
                    <thead>
                        <tr>
                            <th>ID</th>
                    ...
                    <tbody>
                        <?php while ($row = $result_user_book->fetch_assoc()): ?>
                        <tr data-user-id="<?php echo htmlspecialchars($row['user_id'] ?? ''); ?>" data-book-id="<?php echo htmlspecialchars($row['book_id'] ?? ''); ?>">
                            ...
                            <td><?php echo htmlspecialchars($row['borrowed_date'] ?? ''); ?></td>
                            <td><?php echo htmlspecialchars($row['return_due_date'] ?? ''); ?></td>
                            <td><?php echo htmlspecialchars($row['reservation_status'] ?? ''); ?></td>
                            <td>
                                <?php if (($row['status'] ?? '') === 'Onloan'): ?>
                                <button class="btn btn-outline-success btn-sm return-book" data-book-id="<?php echo htmlspecialchars($row['book_id'] ?? ''); ?>" data-user-id="<?php echo htmlspecialchars($row['user_id'] ?? ''); ?>">Return Book</button>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endwhile; ?>
        ...
        document.querySelectorAll('.update-book').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (!editable) {
                    ...
                    row.querySelector('input[type="file"]').classList.remove('d-none');
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            row.querySelector('input[type="file"]').classList.add('d-none');
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.return-book').forEach(button => {
        button.addEventListener('click', function() {
            ...
            formData.append('return_book', true);
